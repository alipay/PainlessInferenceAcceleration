...
class OPTLearnedPositionalEmbedding(nn.Embedding):
    ...
    def forward(self, attention_mask: torch.LongTensor, past_key_values_length: int = 0):
        ...
        if len(attention_mask.shape) == 2:
            ...
            positions = torch.sum(attention_mask, dim=3).squeeze(1) - 1  # [bs, src_len]
...
class OPTAttention(nn.Module):
    ...
    def forward(
        ...
        prefill = False
        use_lookahead = True if decoding_kwargs is not None and decoding_kwargs['use_lookahead'] else False
        # get key, value proj
        if is_cross_attention and past_key_value is not None:
            ...
            if use_lookahead:
                ...
                if decoding_cursors is None:
                    max_len = self.cache_seqlens + tgt_len
                    past_key[:, :,  self.cache_seqlens: max_len] = key_states
                    past_value[:, :, self.cache_seqlens: max_len] = value_states
                else:
                    max_len = max(decoding_cursors) + tgt_len
                    cs = list(set(decoding_cursors))
                    if len(cs) == 1:
                        c = cs[0]
                        past_key[:, :, c: c + tgt_len] = key_states
                        past_value[:, :, c: c + tgt_len] = value_states
                    else:
                        for i, cursor in enumerate(decoding_cursors):
                            past_key[i, :,  cursor: cursor + tgt_len] = key_states[i]
                            past_value[i, :, cursor: cursor + tgt_len] = value_states[i]
                if decoding_cursors is None:
            ...
            if use_lookahead:
                ...
                zeros = torch.zeros([bsz, self.num_heads, decoding_max_length - tgt_len, self.head_dim],
                                    dtype=hidden_states.dtype,
                                    device=hidden_states.device)
                past_key = torch.cat([key_states, zeros], 2)
                past_value = torch.cat([value_states, zeros], 2)
                past_key_value = [past_key, past_value]
                self.cache_seqlens = tgt_len

        if self.is_decoder and not use_lookahead:
        ...
        if not prefill and use_lookahead:
            key_states = past_key[:,:,:max_len]
            value_states = past_value[:,:,:max_len]
...
class OPTPreTrainedModel(LookaheadPreTrainedModel):
