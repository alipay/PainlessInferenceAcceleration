...
class Benchmark():
    ...
    def chat(self, prompt, max_length=256, use_lookahead=False, decoding_length=64, branch_length=8,
             decoding_mode='hier', debug_lookahead=False,chengfazhi=1.0):
    ...
    def batch_chat(self, qs, max_length=256, decoding_length=64, branch_length=8, decoding_mode='hier',
                   debug_lookahead=False, erase=True, batch_size=1,chengfazhi=1.0):
        ...
        for i in range(chat_count // batch_size):
            ...
            for j, use_lookahead in enumerate([False, True]):
                ...
                if use_lookahead:
                    # draft_token_time = kwargs.get('qts', [])  不知道为什么 在数据集测试 这部分数据提取不出来, 感觉服务器没有跳到这部分生成的代码因为改了这部分并没有其他区别显示
                    each_forward_time = kwargs.get('fts', [])
                    # print(draft_token_time)
                    if each_forward_time[1:]:
                        epoch_forward_time=sum(each_forward_time[1:])/len(each_forward_time[1:])
                    else:
                    ...
                    f"edl:{edl:.3f}/dl:{dl:.3f}/forward count:{len(each_forward_time)}/"
                    f"first forward time:{et:.3f}/"
                    # f"find draft time:{sum(draft_token_time):.5f}/"
                    f"epoch time:{epoch_forward_time}/total time:{t:.3f} speed:{speed_token:.1f} speedup:{speedup:.3f}\n")
                else:
                    print(
                        f"{prefix} mode:{decoding_mode} idx:{i}/{chat_count} "
                        f"input:{in_char:.1f}/{in_token:.1f} output:{out_char:.1f}/{out_token:.1f} "
        ...
        # import nni
        # nni.report_intermediate_result(org_speed)
        # nni.report_intermediate_result(opt_speed)
        # nni.report_final_result(speedup)


    def perf_check(self, queries, warmup_ids=None, max_length=256, sizes=(31, 64),
                   lens=(4, 8, 12), decoding_mode='hier',
                   batch_size=1, max_node_rate=32,output_path='test.json',chengfazhi=1.0):
        ...
        for i, decoding_length in enumerate(sizes):
            for j, branch_length in enumerate(lens):
                ...
                for k in range(n_b):
                    ...
                    if (k + 1) % (100 // batch_size) == 0:
                        ...
                        if not use_lookahead:
                            log_str = f'transformers step:{k + 1} ' \
                        ...
                        else: #如果batch size不为1 这部分需要改
                            # draft_token_time = kwargs.get('qts', [])#同理无检索结果
                            # print("ssss",draft_token_time)
                            each_forward_time = kwargs.get('fts', [])
                            log_str = f'lookahead mode:{decoding_mode} step:{k + 1} ' \
                                      f'decoding:{decoding_length}/{branch_length} bs:{batch_size} ' \
                                      f'total time:{elapse:.1f}s in:{avg_in_token:.1f} out:{avg_out_token:.1f} ' \
                                      f'edl:{edl:.3f}/dl:{dl:.3f}/forward count:{len(each_forward_time)}/first forward time:{ft:.3f}'\
                                      f'epoch time:{sum(each_forward_time[1:]) / len(each_forward_time[1:])} speed:{speed_token:.1f}'

                            # f"find draft time:{sum(draft_token_time)}/epoch time:{sum(each_forward_time[1:]) / len(each_forward_time[1:])}/total time:{t:.3f} speed:{speed_token:.1f} speedup:{speedup:.3f}\n")
                ...
                if not use_lookahead:
                    log_str = f"transformer bs:{batch_size} " \
                          f"query:{len(queries)} " \
                          f"input:{in_token:.1f} output:{out_token:.1f} " \
                          f"edl:{edl:.3f}/{dl:.3f}/{ft:.3f} time:{t:.3f} " \
                          f"speed:{speed:.1f} mem:{mem:.3f} "
                    log_data = {
                        "dataset_name": "mt_bench",  # 数据集名称
                        "dataset_size": len(queries),  # 数据集个数
                        "avg_decode_length": out_token,  # 解码长度（平均输出 token 数）
                        "avg_infer_time": t,  # 平均推理时间（单位：秒）
                        "speed": speed,  # 每秒生成 token 数（token/s）
                        "mem_usage_gb": mem,  # GPU内存使用（单位：GB）
                        "requests_per_second": len(queries) / t  # 每秒处理请求数（query/s）
                    }
                else:
                    log_str = f"lookahead mode:{decoding_mode} bs:{batch_size} " \
                    ...
                    log_data = {
                                ...
                                "mem_usage_gb": mem,  # GPU内存使用（单位：GB）
                                "requests_per_second": len(queries) / t,  # 每秒处理请求数（query/s）
                                "repetition_penalty": chengfazhi,  # 惩罚值
                                "decoding_length": decoding_length,  # 树解码长度
                                "branch_length": branch_length  # 分支长度
                            }

                import json
                import nni
                nni.report_final_result(speed)
