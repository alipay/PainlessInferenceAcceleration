...
from pia.lookahead.models.baichuan2_7b.generation_utils import build_batch_chat_input, TextIterStreamer
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # print(f'{hidden_states.size()=} {position_ids.shape=}')
        ...
        if past_key_value is None:
            ...
            zeros = torch.zeros([bsz, self.num_heads, decoding_max_length - l, self.head_dim],
...
class BaichuanForCausalLM(BaichuanPreTrainedModel):
    ...
    def chat(self, tokenizer, messages: List[dict], stream=False,
        ...
        inputs = build_batch_chat_input(self, tokenizer, messages, generation_config.max_new_tokens)
        input_ids = inputs.input_ids.cuda()
        attention_mask = inputs.attention_mask.cuda()
        # position_ids = inputs.position_ids.cuda()
        # position_ids = torch.maximum(torch.cumsum(attention_mask,dim=1)-1, torch.zeros(()).long().cuda())
        outputs = self.generate(input_ids,attention_mask=attention_mask, generation_config=generation_config)
        response = tokenizer.batch_decode(outputs[:,input_ids.size(1):], skip_special_tokens=True)
        return response
