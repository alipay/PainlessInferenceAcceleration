...
def lookahead_build_alibi_tensor(attention_mask: torch.Tensor, num_heads: int, dtype: torch.dtype) -> torch.Tensor:
    """
    Link to paper: https://arxiv.org/abs/2108.12409 Alibi tensor is not causal as the original paper mentions, it
    relies on a translation invariance of softmax for quick implementation: with l being a tensor, and a fixed value
    `softmax(l+a) = softmax(l)`. Based on
    ...
    Returns tensor shaped (batch_size * num_heads, 1, max_seq_len)
        ...
        dtype (`torch.dtype`, *optional*, default=`torch.bfloat16`):
            dtype of the output tensor
    """
    batch_size, _, q_len, seq_length = attention_mask.shape
    closest_power_of_2 = 2 ** math.floor(math.log2(num_heads))
    base = torch.tensor(
        2 ** (-(2 ** -(math.log2(closest_power_of_2) - 3))), device=attention_mask.device, dtype=torch.float32
    )
    ...
    # This is more or less identical to T5's relative position bias:
    # https://github.com/huggingface/transformers/blob/f681437203baa7671de3174b0fa583c349d9d5e1/src/transformers/models/t5/modeling_t5.py#L527
    arange_tensor = ((attention_mask.cumsum(dim=-1) - 1) * attention_mask)  # [bs,1,q_len,kv_len]
    alibi = slopes[..., None, None] * arange_tensor  # []
    return alibi.reshape(batch_size * num_heads, q_len, seq_length).to(dtype)
...
class BloomAttention(nn.Module):
    ...
    #         # print(f'{query_layer.shape=} {key_layer.shape=} {past_key.shape=} {value_layer.shape=} {past_value.shape=}')
    ...
    #         batch1=query_layer.view(batch_size*self.num_heads, q_length, self.head_dim),
    #         batch2=key_layer.reshape(batch_size*self.num_heads, kv_length, self.head_dim).transpose(1,2),
    #         beta=self.beta,
    ...
    #     context_layer = torch.bmm(attention_probs_reshaped, value_layer.view(batch_size*self.num_heads, kv_length, self.head_dim))
    ...
    def forward(
        ...
        # aggregate results across tp ranks. See here: https://github.com/pytorch/pytorch/issues/76232
        if hasattr(self, 'pretraining_tp') and self.pretraining_tp > 1 and self.slow_but_exact:
            slices = self.hidden_size / self.pretraining_tp
            output_tensor = torch.zeros_like(context_layer)
            for i in range(self.pretraining_tp):
                output_tensor = output_tensor + F.linear(
...
class BloomPreTrainedModel(LookaheadPreTrainedModel):
