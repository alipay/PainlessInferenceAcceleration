...
class GLMBlock(nn.Module):
    ...
    def forward(
        ...
        attn_output, attn_weight, past_key_value = self.attention(
            hidden_states,
        ...
        return outputs,attn_weight,past_key_value
...
class GLMStack(torch.nn.Module):
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.layers, past_key_values)):
            ...
            if use_cache:
                past_key_values.append(outputs[2])
...
class GLMModel(GLMPreTrainedModel):
    ...
    def forward(
        ...
        if inputs_embeds is not None and past_key_values is None:
            inputs_embeds = inputs_embeds.to(device=words_embeddings.device, dtype=words_embeddings.dtype)
