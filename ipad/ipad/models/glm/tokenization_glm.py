...
def encode_whitespaces(content):
    for i in range(10, 1, -1):
        content = content.replace(' '*i, f'<|blank_{i}|>') # noqa
    return content


def decode_whitespaces(content):
    for i in range(10, 1, -1):
        content = content.replace(f'<|blank_{i}|>', ' '*i) # noqa
    return content
...
class GLMBatchEncoding(BatchEncoding):
    def to(self, device: Union[str, "torch.device"]) -> "BatchEncoding":
        ...
        if isinstance(device, str) or _is_torch_device(device) or isinstance(device, int):
            self.data = {k: v.to(device=device) if torch.is_tensor(
                v) else v for k, v in self.data.items()}
        else:
            logger.warning(
...
class GLMTokenizerMixin:
    ...
    def _build_input_for_multiple_choice(self, context, choices):
        ...
        for choice_str in choices:
            ...
            choice_indices.append(torch.arange(
                len(token), len(token) + len(choice), dtype=torch.long))
            attention_mask.append(torch.tril(torch.ones(
                (len(choice), len(choice)), dtype=torch.long)))

            token = torch.cat((token, torch.tensor(
                [self.sop_token_id], dtype=torch.long), choice[:-1]))
            position_id = torch.cat((position_id, torch.tensor(
                [mask_position] * len(choice), dtype=torch.long)))
            block_position_id = torch.cat(
                (block_position_id, torch.arange(1, 1 + len(choice), dtype=torch.long)))

        attention_mask = torch.block_diag(*attention_mask)
        attention_mask[division:,
                       :division] = context["attention_mask"].unsqueeze(0)
    ...
    def build_inputs_for_multiple_choice(self, model_input: BatchEncoding, choices, max_length=None):
        samples = [{key: value[i] for key, value in model_input.items()}
    ...
    def build_inputs_for_generation(self, model_input: BatchEncoding, max_gen_length=512, targets=None, padding=False):
        ...
        position_id, block_position_id = list(range(seq_length)), [
            0 for _ in range(seq_length)]
        position_ids, block_position_ids = [], []
        labels = None
        if targets is not None:
            is_batched = isinstance(targets, (list, tuple))
            targets = self(targets, add_special_tokens=False,
                           padding=False).input_ids
            if not is_batched:
                targets = [targets]
            assert len(targets) == len(input_ids)
            targets = [(target + [self.eop_token_id])[:max_gen_length]
            ...
            targets = [target + [self.pad_token_id] *
                       (max_gen_length + 1 - len(target)) for target in targets]
            labels = [label + [-100] * (max_gen_length - len(label))
                      for label in labels]
            targets = torch.tensor(
                targets, dtype=input_ids.dtype, device=input_ids.device)
            labels = torch.tensor(
                labels, dtype=input_ids.dtype, device=input_ids.device)
            labels = torch.cat(
                (input_ids.new_full((batch_size, seq_length), -100), labels), dim=1)
        for i in range(batch_size):
            mask_positions = []
            for mask_id in mask_ids:
                mask_positions += (input_ids[i] ==
                                   mask_id).nonzero(as_tuple=True)[0].tolist()
            if not mask_positions:
                raise ValueError("Cannot find mask token in the input")
            mask_positions.sort()
            mask_pos = mask_positions[0]
            position_ids.append(position_id + [mask_pos] * max_gen_length)
            block_position_ids.append(
                block_position_id + list(range(1, max_gen_length + 1)))
        position_ids = torch.tensor(
            position_ids, dtype=input_ids.dtype, device=input_ids.device)
        block_position_ids = torch.tensor(
            block_position_ids, dtype=input_ids.dtype, device=input_ids.device)
        position_ids = torch.stack((position_ids, block_position_ids), dim=1)
        attention_mask = model_input.attention_mask
        attention_mask = attention_mask.unsqueeze(
            1).expand(-1, seq_length + max_gen_length, -1)
        ...
        attention_mask = torch.cat(
            (attention_mask, generation_attention_mask), dim=2)
        attention_mask = attention_mask.unsqueeze(1)
        if targets is None:
            input_ids = torch.cat((input_ids, input_ids.new_full(
                (batch_size, 1), self.sop_token_id)), dim=-1)
        else:
...
class GLMChineseTokenizer(PreTrainedTokenizer, GLMTokenizerMixin):
    ...
    def convert_tokens_to_string(self, tokens):
        return decode_whitespaces(self.sp_model.decode(tokens))
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        if config_tokenizer_class == "GLMRobertaTokenizer":
            ...
            raise NotImplementedError(
                "Not implemented tokenizer type:", config_tokenizer_class)
