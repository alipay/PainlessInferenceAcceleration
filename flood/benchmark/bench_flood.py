...
if __name__ == "__main__":
    ...
    reqs = Reader.read_dummy_dataset(
        max_count=10000, input_length=128, output_length=128, flunc=0.1
    )
    ...
    for i, req in enumerate(
        worker.request_stream_generate(reqs, input_queue, output_queues, print_count=3)
    ):
        f.write(
            json.dumps(
                {"id": req.rid, "prompt": req.input_text, "response": req.output_text}
            )
            + "\n"
        )
        if i <= -1:
