...
def test_seg_attn(mode="prefill", even=True, decouple=False):
    ...
    if mode == "prefill":
        org_output, state_ref = chunk_simple_gla(
        ...
        org_output, state_ref = fused_recurrent_simple_gla(
            q.view(bs, qls[0], qo_head, dim),
            k.view(bs, kls[0], kv_head, dim),
            v.view(bs, kls[0], kv_head, dim),
            decay_scales.view(1, 1, -1).expand(bs, qls[0], qo_head),
    ...
    n_repeat = 1000
    benchmark_func(
        flash_attn, q, k_fa, v_fa, flash_attn_meta, ref_flops=flops, n_repeat=n_repeat
    )
