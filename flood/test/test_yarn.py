...
from transformers.models.deepseek_v2.modeling_deepseek_v2 import (
    ...
    apply_rotary_emb as ds_v2_apply_rotary_emb,
    DeepseekV2Config,
)
...
class NativeDeepseekYarnRope(torch.nn.Module):
    def __init__(self, config, is_neox_style=True):
        ...
        self.low, self.high = self._yarn_find_correction_range(
            self.beta_fast,
            self.beta_slow,
            self.rotary_dim,
            self.rope_theta,
    ...
    def _compute_inv_freq(self, scaling_factor: float) -> torch.Tensor:
        ...
        inv_freq_mask = 1 - self._yarn_linear_ramp_mask(
            self.low, self.high, self.rotary_dim // 2, dtype=torch.float
        )
        inv_freq = (
    ...
    def _yarn_linear_ramp_mask(
        self, low: float, high: float, dim: int, dtype: torch.dtype
    ) -> torch.Tensor:
        if low == high:
