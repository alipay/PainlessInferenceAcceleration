...
import random
import torch
from flood.ops.rope import (
    triton_qk_norm_and_rope_forward,
    triton_q_k_norm_and_rope_forward,
)
...
def torch_rope(
    ...
    qr, kr = apply_rotary_pos_emb(
        q[:, :, :, :rotary_dim], k[:, :, :, :rotary_dim], cos, sin, position_ids
    )
...
def test_qk_norm_and_rope(
    ...
    q_ref, k_ref, v_ref = torch_qk_norm_and_rope(
        qkv,
        qw,
        kw,
        position_ids,
    ...
    if bench:
        ...
        benchmark_func(
            ...
            eps=1e-6,
            n_repeat=1000,
            ref_bytes=ql * B * (H + 2 * h) * D * 4,
            n_profile=0,
            trace_dir="./cuda_qknorm_rope.json",
        )
        benchmark_func(
            triton_qk_norm_and_rope_forward,
            qkv_clone,
            ...
            ref_bytes=ql * B * (H + 2 * h) * D * 4,
            n_profile=0,
            trace_dir="./triton_qknorm_rope.json",
        )
        benchmark_func(
            triton_q_k_norm_and_rope_forward,
            q,
            ...
            ref_bytes=ql * B * (H + 2 * h) * D * 4,
            n_profile=0,
            trace_dir="./triton_q_knorm_rope.json",
        )
