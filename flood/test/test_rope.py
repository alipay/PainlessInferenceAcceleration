...
from transformers.models.llama.modeling_llama import LlamaRotaryEmbedding, \
    apply_rotary_pos_emb as llama_apply_rotary_pos_emb, LlamaConfig
from transformers.models.gpt_neox.modeling_gpt_neox import GPTNeoXRotaryEmbedding, \
    apply_rotary_pos_emb as gpt_neox_apply_rotary_pos_emb, GPTNeoXConfig
...
if __name__ == '__main__':
    ...
    q_rope_ref, k_rope_ref = llama_apply_rotary_pos_emb(q, k, cos, sin,
                                                  unsqueeze_dim=2)
    ...
    conf = GPTNeoXConfig(hidden_size=qo_head*dim,num_attention_heads=qo_head,rope_theta=rope_theta,max_position_embeddings=4096, rotary_pct=partial_rotary_factor)
    rotary_emb = GPTNeoXRotaryEmbedding(conf, device=device)
    cos, sin = rotary_emb(q, position_ids)
    q_gpt_ref, k_gpt_ref = gpt_neox_apply_rotary_pos_emb(q, k, cos, sin,
                                                  unsqueeze_dim=2)
