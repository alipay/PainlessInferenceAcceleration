...
def test_dynamic_scaled_fp8_quant(num_tokens: int, hidden_size: int,
    ...
    ker_scales = torch.empty((shape[0], 1),
                             device=x.device,
                             dtype=torch.float32)
    ...
    torch.testing.assert_close(ker_out.float(), ref_out.float(), atol=0.01,
                               rtol=0.01)
