...
def scaled_dot_product_attention_fp32(query, key_value, softmax_scale):
    ...
    mask = torch.tril(
        torch.ones(q_length, k_length, dtype=torch.float32, device=query.device),
        k_length - q_length,
    )
    # print(mask)
...
def test_seg_mla(max_seg=1, mode="prefill", even=True):
    ...
    for i, ql in enumerate(qls):
        ...
        flops += (ql * ql + ql * (kvl - ql) * 2) * 128 * 576 * 2
    ...
    for i, qi in enumerate(qs):
        kvi = kvs[i]
        org_output, org_lse = scaled_dot_product_attention_fp32(
            qi.view(1, qls[i], 128, 576), kvi.view(1, kls[i], 1, 576), softmax_scale
        )
    ...
    if math.isnan(rate) or rate > 0.02:
        ...
        n_repeat = 100
        benchmark_func(
            seg_mla_fwd, q, kv, seg_attn_meta, ref_flops=flops, n_repeat=n_repeat
        )
