...
def scaled_dot_product_attention(query, key_value):
    ...
    attn_weight = query @ key / math.sqrt(q_dim)  # [bs, 128, q_length, k_length]
    mask = torch.tril(
        torch.ones(q_length, k_length, dtype=query.dtype, device=query.device),
        k_length - q_length,
    )
    # print(mask)
    attn_weight -= 10000 * (1 - mask)
...
if __name__ == "__main__":
    ...
    output_err = (
        (ref_output - opt_output).abs().mean() / ref_output.abs().mean()
    ).item()
    ...
    benchmark_func(
        ...
        kv,
        n_repeat=100,
        ref_flops=bs * q_length * k_length * 128 * (576 + 512) * 2 / 2,
    )
