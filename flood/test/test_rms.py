...
def torch_rms_norm(x, w, eps=1e-6):
    output = x * torch.rsqrt(x.float().pow(2).mean(-1, keepdim=True) + eps).to(x.dtype)
    return output * w
...
benchmark_func(
    test_group_rms_norm_silu, x, w, g, eps, hidden_size, n_group, n_repeat=1000
)

benchmark_func(
    triton_rms_groupnorm_sigmoid,
    x,
    w,
    g,
    eps,
    ...
    n_repeat=1000,
    n_profile=10,
    trace_dir="./triton_rms_groupnorm_sigmoid.json",
)
