...
from flood.ops.norm import RMSNorm, RMSGroupNormSigmoid
...
class BailingMoeV2MLP(torch.nn.Module):
...
class BailingMoeV2MoE(torch.nn.Module):
    def __init__(self, config: PretrainedConfig, layer_idx: int = 0):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        ...
        modules = torch.nn.ModuleList(
            [BailingMoeV2MLP(exp_conf, layer_idx=-1) for _ in range(self.num_experts)]
        )
        ...
        self.gate = AutoLinear.from_pretrained(
            config.hidden_size, self.num_experts, bias=False, config=config, name="gate"
        )
...
class BailingMoeV2Attention(torch.nn.Module):
    def __init__(self, config: PretrainedConfig, layer_idx: int = 0):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        ...
        self.query_key_value = AutoLinear.from_pretrained(
            self.hidden_size,
            qkv_dim,
        ...
        self.dense = AutoLinear.from_pretrained(
            self.intermediate_size,
            self.hidden_size,
...
class BailingMoeV2LinearAttention(torch.nn.Module):
    def __init__(self, config: PretrainedConfig, layer_idx: int = 0):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        ...
        self.g_proj = AutoLinear.from_pretrained(
            self.hidden_size, self.num_heads * self.head_dim, bias=False, config=config
        )
        self.query_layernorm = RMSNorm(self.head_dim, eps=config.rms_norm_eps)
        self.key_layernorm = RMSNorm(self.head_dim, eps=config.rms_norm_eps)
        self.g_norm = RMSGroupNormSigmoid(
    ...
    def flood_patch_func(self, kwargs=None):
        ...
        self.query_key_value_gate = self.query_key_value.merge(
            [self.query_key_value, self.g_proj]
        )
        ...
        self.decay_scales = torch.pow(start, exponents) * (
...
class BailingMoeV2DecoderLayer(torch.nn.Module):
    def __init__(self, config: PretrainedConfig, layer_idx: int = 0):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class BailingMoeV2Model(PreTrainedModel):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        if self.rank == 0:
            self.word_embeddings = AutoEmbedding.from_pretrained(
                config,
                config.vocab_size,
                config.hidden_size,
        ...
        for i in range(n_layer):
            ...
            layers.append(BailingMoeV2DecoderLayer(config, layer_idx=layer_idx))
