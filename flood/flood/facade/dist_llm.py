...
class DistLLM(LLM):
            ...
            if n_layer % (n_stage * self.world_size) == 0:
                if n_stage == -1:
                    ...
                    counts = [
                        ...
                        m + 1,
                        m,
                    ] * self.world_size  # set n_proc = n_stage
                else:
    ...
    def get_cache_size(self, n_stage, max_rate, token_size, device_list):
        ...
        cache_size = (
            int(
                (min(totals) * max_rate - max(usages))
                / token_size
                * n_stage
                / self.world_size
                / 16
    ...
    def pingpong_schedule(
        ...
        dist.init_process_group(
            backend="nccl", init_method=http, world_size=self.world_size, rank=self.rank
        )
        group = None
        ...
        while True:
            ...
            if self.rank == 0:
                ...
                dist.recv_object_list(
                    objects, src=self.world_size - 1, group=group, device=comm_device
                )
                objects = objects[0]
                mode = objects[0]
                reqs = objects[1]
                    ...
                    if req.todo > 0 and req.done + req.todo < req.input_length:
                        ...
                        if req.target_ids is None:
                            ...
                            if (
                                ...
                                req.input_length + len(req.output_ids)
                                >= req.size_of_segs()
                            ):
                                print(
                ...
                if self.debug:
                    ...
                    mean_output_length = sum(output_lengths) / max(
