...
class LLM:
    def __init__(
        ...
        (
            ...
            self.kv_heads,
            self.head_dim,
        ) = Reader.get_conf(self.model_path)
        ...
        if "layer_group_size" in self.conf:
            ...
            ), "max_concurrency must be set for linear model"
            self.layer_group_size = self.conf["layer_group_size"]
            self.fix_size_indices = [
                i for i in range(self.n_layer) if (i + 1) % self.layer_group_size != 0
            ]
        ...
        self.model = self.load_model(self.model_path, self.device_list, self.n_stage)
        ...
        if eos_token_id is None:
            ...
            eos_token_id = tuple(set([x for x in eos_token_ids if x is not None]))
        ...
        if cache_size < 1:
            token_size = 2 * self.kv_heads * self.head_dim * self.cache_dtype.itemsize
            self.cache_size = self.get_cache_size(
                self.n_stage, cache_size, token_size, self.device_list
            )
            print(
        ...
        if self.spec_algo == "lookahead":
            ...
            self.spec = Lookahead(
                table_size=self.spec_table_size // self.spec_branch_length // 4,
    ...
    def load_model(self, model_path, device_list, n_stage):
        ...
        model = Model.from_pretrained(
            model_path, dtype=torch_dtype, low_cpu_mem_usage=True, device_map=device_map
        )
        ...
        if self.model_dtype == torch.float8_e4m3fn:
            ...
            for name, module in model.named_modules():
                if isinstance(module, NativeLinear):  # TODO: adapt for MOE
                    self.set_module_by_name(
                        model, name, module.retype(dtype=torch.float8_e4m3fn)
                    )
    ...
    def get_cache_size(self, n_stage, max_rate, token_size, device_list):
        ...
        for i in range(n_stage):
            ...
            if self.max_concurrency is not None:
                n_var_size_layer = sum(
                    1 for j in device_list[i] if j not in self.fix_size_indices
                )
                n_fix_size_layer = len(device_list[i]) - n_var_size_layer
                alloc_memory = (
                    max_rate * total_memory
                    - used_memory
                ...
                n_var_size_layer = len(device_list[i])
            cache_size = alloc_memory / (token_size * n_var_size_layer)
            cache_sizes.append(cache_size)
            info.append(
                f"{total_memory/2**30:.1f}/{used_memory/2**30:.1f}/{free_memory/2**30:.1f}"
            )
        info = " ".join(info)
    ...
    def init_kv_cache(self, max_token, cache_dtype):
        ...
        if (
            self.model_type == "DeepseekV3ForCausalLM"
            or self.model_type == "DeepseekV2ForCausalLM"
        ):
    ...
    def launch(self, input_queue, chunk_quene, working_queue, output_queue):
        ...
        slots = Array(
            Slot,
            [(0, self.cache_size - 128, 1, 0)]
            + [(0, 0, 0, 0) for i in range(self.slot_count)],
        )
    ...
    def pingpong_schedule(
        ...
        while True:
            ...
            input_empty = (
                input_queue.empty()
                and fail_sample_count.value == 10**self.n_proc
                and len(chunks) == 0
                and len(options) == 0
            )
            working_empty = (
                working_queue.empty() and len(waits) == 0 and counts.value == 0
            )  # TODO: CHECK counts.value
            ...
            if (
                task_id != 0
                and input_empty
                and counts.value <= self.min_batch_size
                and self.spec_algo is None
            ):
                time.sleep(0.001)
                continue
            ...
            if state.value == 0:
                ...
                if n_tokens < chunk_size:
                    while True:
                        try:
                            req = input_queue.get(block=True, timeout=queue_timeout)
                        except:
                            break
                        # assert req.done == 0
                ...
                if len(output_lengths) >= 512 and self.tune_alloc_size:
                    fully_alloc_under = sorted(output_lengths)[
                        int(0.9 * len(output_lengths))
                    ]

                batch = Batch.prefill_batching(
                    reqs,
                    slots,
                    fix_slots=fix_slots,
                    device=input_device,
                ...
                if batch.batch_size > 0:
                    ...
                    LLM.update_digit(
                        fail_sample_count, task_id + 1, len(fails) + len(chunks)
                    )
                else:
                    task_type = None
                    # fail to allocate slot, should reput the reqs into inputs
                    sizes = [
                        str(x.todo or x.input_length) + "+" + str(x.output_length)
                        for x in reqs
                    ]
                    sizes = ",".join(sizes)
                ...
                for req in waits:
                    if len(reqs) < dbs:
                        ...
                        assert (
                            req.input_length + len(req.output_ids) == size_of_segs + 1
                        )
                        slot_size = req.input_length + req.output_length + self.spec_buf
                        extend_length = min(
                            slot_size - size_of_segs, self.max_extend_size
                        )
                        slot_size = ((extend_length - 1) // 128 + 1) * 128
                        segs = Batch.extend_slot(
                            slots,
                            req.segs,
                            extend_length,
                        ...
                        if segs is not None:
                            ...
                            if "sa" in self.kernels:  # no segment avaible
                                ...
                                if (
                                    allocate_fail_count.value
                                    >= self.max_slot_alloc_fail_count
                                ):
                                    gbs.value = self.opt_batch_size(
                                        counts.value // 2, self.n_proc
                                    )
                                    state.value = min(
                                        int(self.min_decode_rate * counts.value),
                                        gbs.value * self.n_proc,
                ...
                if self.spec_algo == "lookahead" and hungry:
                    min_buf = min(
                        [
                            x.size_of_segs() - x.input_length - len(x.output_ids)
                            for x in reqs
                        ]
                    )
                    if min_buf >= self.spec_branch_length:
                        max_retrieve_count = min(
                            min_buf // self.spec_branch_length,
                            self.max_spec_branch_count,
                        )
                        retrieve_count = min(
                            max(
                                64
                        ...
                        with torch.cuda.stream(batching_stream):
                            batch = Batch.lookahead_batching(
                                reqs,
                                self.spec,
                                device=input_device,
            ...
            for i, req in enumerate(batch.reqs):
                if req.todo > 0 and req.done + req.todo < req.input_length:
                    ...
                    if req.target_ids is None:
                        ...
                        if task_type == "spec":
                            for j in range(
                                max(-self.spec_branch_length - 1, -len(req.output_ids)),
                                -1,
                            ):
                        ...
                        if (
                            len(req.output_ids) >= req.output_length
                            ...
                            req.input_length + len(req.output_ids) > req.size_of_segs()
                        ):
                            waits.append(req)
                        else:
                        ...
                        if isinstance(req.target_ids, list):
                            if req.todo == 0:  # not chunked
                                ...
                                if req.done + req.todo < req.input_length + sum(
                                    [len(x) for x in req.target_ids]
                                ):  # unfinished
                                    req.done += req.todo
                                    req.todo = 0
                                    options.append(req)
                            ...
                            output_queue.put(req)
                            Batch.recycle(
                                slots, req.segs, fix_slots, req.fix_size_slot_index
                            )
                            counts.value -= 1
                        else:
                            working_queue.put(req)
    ...
    def mix_schedule(
        ...
        while True:
            ...
            # both empty, wait
            input_empty = (
                input_queue.empty() and fail_sample_count.value == 10**self.n_proc
            )
            ...
            batch = Batch.mix_batching(
                reqs, slots, device=input_device, min_rate=self.alloc_early_exit_rate
            )

            if batch.batch_size > 0:
                counts.value += sum([x.task_type == 0 and x.done == 0 for x in reqs])
                LLM.update_digit(
                    fail_sample_count, task_id + 1, len(fails) + len(chunks)
                )
            else:
                ...
                sizes = [
                    str(x.input_length) + "+" + str(x.output_length)
                    for x in reqs
                    if x.task_type == 0 and x.done == 0
                ]
                sizes = ",".join(sizes)
                print(
                    f"No slots available! task:{task_id} pid:{os.getpid()} "
                    f"ips:{input_queue.qsize()} counts:{counts.value} state:{state.value} "
                    f"size:{sizes}"
                )
                for req in reqs:
    ...
    def timely_schedule(
        ...
        while True:
            ...
            if (
                not input_empty
                and counts_value <= state.value
                and (input_size > 1 or prefill_gap_time >= min_gap_time)
            ):
                state.value = 0
            elif input_empty or input_size == 1 and prefill_gap_time < min_gap_time:
                gbs.value = self.round_batch_size(
                    counts_value, self.batch_size_round_frac
                )
                # upper bound to do prefill asap
                state.value = self.n_proc * self.round_batch_size(counts_value, 1.0)
            dbs = gbs.value
            ...
            if state_value == 0:
                ...
                if batch.batch_size > 0:
                    ...
                    sizes = [
                        str(x.input_length) + "+" + str(x.output_length) for x in reqs
                    ]
                    sizes = ",".join(sizes)
                    print(
                        f"No slots available! task:{task_id} pid:{os.getpid()} dbs:{dbs} "
                    ...
                    if allocate_fail_count.value >= self.max_slot_alloc_fail_count:
                        gbs.value = self.round_batch_size(
                            counts.value, self.batch_size_round_frac
                        )
                        # state.value = int(self.min_decode_rate * counts.value)
                        state.value = max(
                            int(self.min_decode_rate * counts.value), counts.value - 16
                        )
                        allocate_fail_count.value = 0
                    continue
            else:
            ...
            for i, req in enumerate(batch.reqs):
                if req.todo > 0 and req.done + req.todo < req.input_length:
                    ...
                    if task_type == "spec":
                        for j in range(
                            max(-self.spec_branch_length - 1, -len(req.output_ids)), -1
                        ):
                            if req.output_ids[j] in self.eos_token_id:
                                req.output_ids = req.output_ids[: j + 1]
                                break
                    ...
                    if (
                        ...
                        req.output_ids.append(
                            None
                        )  # NOTE: MUST USE NONE WITH stream MODE
                        ...
                        if self.spec_algo == "lookahead":
                            self.spec.update_state(
                                req.output_ids[:-1]
                            )  # the last one is None
                        counts.value -= 1
                        ...
                        if (
                            self.spec_algo is not None
                            or (len(req.output_ids) - 1) % 10 == 0
                        ):
                            output_queues[req.output_index].queue.put(req)
                        working_queue.put(req)
            ...
            if self.debug:
                ...
                tokens = batch.token_count
                bs_str = (
                    f"{batch.batch_size}/{tokens}"
                    if batch.mode == 0
                    else f"{dbs}/{batch.batch_size}"
                )
    ...
    def generate(
        self, requests: List, input_queue, output_queues, print_count=0, log_info=""
    ):
        responses = []
        for response in self.request_stream_generate(
            requests,
            input_queue,
            output_queues,
    ...
    def request_stream_generate(
        ...
        total_slot_size = 0
        for req in requests:
            input_ids = tokenizer(req.input_text, add_special_tokens=False)["input_ids"]
            req.input_ids = input_ids
            req.input_length = len(input_ids)
            output_slot_size = req.output_length
            slot_size = ((req.input_length + int(output_slot_size) - 1) // 16 + 1) * 16
            r = Req(
                req.rid,
                input_ids=input_ids,
        ...
        while output_sample_count < n_sample:
            ...
            if len(output_ids) > 0 and isinstance(output_ids[0], list):
                ...
                output_text = " ".join(
                    [
                        target_tokens[i] + ":" + str(x)
                        for i, x in enumerate(output_ids[0])
                    ]
                )
            else:
            ...
            if output_sample_count % 100 == 0:
                ...
                batch_tokens = sum([len(x.output_ids) for x in responses[-100:]])
    ...
    def opt_batch_size(self, value, n_proc):
        ...
        if bs >= self.batch_size_step:
            bs = (
                int(value / n_proc / self.batch_size_step + self.batch_size_round_frac)
    ...
    def tokenize(self, requests, input_queue, qps=None):
        ...
        for req in requests:
            ...
            if qps is not None:
                time.sleep(1.0 / qps)
        te = time.time()
        print(f"pid:{os.getpid()} tokenize:{te - ts:.3f}s ts:{time.time() % 1000:.3f}")

    def stream_generate(self, request: Request, input_queue, output_queues):
    ...
    async def async_stream_generate(self, request: Request, input_queue, output_queue):
    ...
    def round_batch_size(self, count_value, frac=0.0):
        if self.batch_size_step is None:
            value = min(
                max(
                    2 ** int(math.log2(max(count_value / self.n_proc, 1)) + frac),
                    self.min_batch_size,
                ),
                self.max_batch_size,
            )
            ...
            value = min(
                max(
                    int(max(count_value / self.n_proc, 1) / self.batch_size_step + frac)
                    * self.batch_size_step,
                    self.min_batch_size,
                ),
                self.max_batch_size,
    ...
    def format_params(self, n_sample, log_info=""):
        ...
        device = str(self.n_stage) + "*" + torch.cuda.get_device_name().split(" ")[-1]
    ...
    def get_emb(self, req, fe):
        ...
        if req.done >= req.emb_idx + req.emb_size or req.done + req.todo <= req.emb_idx:
            return None
        return fe.get_tensor(req.rid)
