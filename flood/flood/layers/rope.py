...
class AutoRope(torch.nn.Module):
    ...
    def from_pretrained(cls, config):
        if (
            ...
            or isinstance(config.rope_scaling, (int, float))
        ):
            return NativeRope(config)
        assert isinstance(config.rope_scaling, dict)
        rope_type = config.rope_scaling.get("rope_type") or config.rope_scaling.get(
            "type"
        )
        if rope_type == "yarn":
...
class YarnRope(torch.nn.Module):
    def __init__(self, config):
        ...
        self.low, self.high = _yarn_find_correction_range(
            self.beta_fast,
            self.beta_slow,
            head_dim,
            self.rope_theta,
...
class DeepseekYarnRope(torch.nn.Module):
    def __init__(self, config):
        ...
        self.mscale_all_dim = config.rope_scaling.get("mscale_all_dim", 1.0)
        self.magnitude_scale = float(
            yarn_get_mscale(self.rope_scale, float(self.mscale))
            / yarn_get_mscale(self.rope_scale, float(self.mscale_all_dim))
        )
