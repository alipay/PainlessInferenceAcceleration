...
def fused_moe_kernel(
    ...
    if has_bias:
        bias_ptrs = bias_ptr + off_experts * stride_be_bias + offs_bn * stride_bn_bias
    ...
    if has_bias:
        bias = tl.load(bias_ptrs, mask=True, other=0.0)
...
def moe_align_block_size(
    ...
    num_tokens_post_pad = torch.empty((1),
                                      dtype=torch.int32,
                                      device=topk_ids.device)
    flood_cuda.moe_align_block_size(topk_ids, num_experts, block_size,
                                    sorted_ids,
...
class Fp8Experts(torch.nn.ModuleList):
    ...
    def _flood_patch_func(self):
        ...
        for expert in iter(self._modules.values()):
            w1.append(torch.cat([expert.gate_proj.weight.data.view(torch.int8),
                                 expert.up_proj.weight.data.view(torch.int8)],
                                dim=0))
            w2.append(expert.down_proj.weight.data.view(torch.int8))

            w1_scale.append(torch.cat([expert.gate_proj.weight_scale.data,
                                       expert.up_proj.weight_scale.data],
                                      dim=0))
            w2_scale.append(expert.down_proj.weight_scale.data)

            a1_scale.append(torch.max(expert.gate_proj.input_scale.data,
