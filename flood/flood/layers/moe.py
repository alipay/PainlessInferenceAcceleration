...
def moe_align_block_size(
    ...
    flood_cuda.moe_align_block_size(
...
def fused_experts(
    ...
    for chunk in range((num_tokens // CHUNK_SIZE) + 1):
        ...
        flood_cuda.silu_and_mul(intermediate_cache2, intermediate_cache1.view(-1, N))
...
class AutoExperts:
    ...
    def from_pretrained(
        ...
        if isinstance(module_list, torch.nn.ModuleList):
            if dtype is None:
                return NativeExperts(
                    module_list, scoring_func, num_expert_group, topk_group
                )
            ...
            if dtype is None:
                return StackExperts(
                    hidden_size, intermediate_size, num_expert, dtype, config
                )
            elif dtype == torch.float8_e4m3fn:
                return StackFp8Experts(
                    hidden_size, intermediate_size, num_expert, dtype, config
                )
            else:
                raise ValueError(f"unknown dtype:{dtype}")
...
class NativeExperts(torch.nn.ModuleList):
    def __init__(self, modules, scoring_func, num_expert_group, topk_group) -> None:
        ...
        self.use_grouped_topk = (num_expert_group is not None) and (
            topk_group is not None
        )
    ...
    def _flood_patch_func(self):
        ...
        for expert in iter(self._modules.values()):
            w1.append(
                torch.cat(
                    [expert.gate_proj.weight.data, expert.up_proj.weight.data], dim=0
                )
            )
...
class Fp8Experts(torch.nn.ModuleList):
    ...
    def _flood_patch_func(self):
        ...
        for expert in iter(self._modules.values()):
            ...
            if (
                hasattr(expert.gate_proj, "weight_scale")
                and hasattr(expert.up_proj, "weight_scale")
                and hasattr(expert.down_proj, "weight_scale")
            ):
                w1_scale.append(
                    torch.cat(
                        [
                ...
                hasattr(expert.gate_proj, "weight_scale_inv")
                and hasattr(expert.up_proj, "weight_scale_inv")
                and hasattr(expert.down_proj, "weight_scale_inv")
            ):
                w1_scale.append(
                    torch.cat(
                        [
            ...
            if (
                expert.gate_proj.input_scale
                and expert.up_proj.input_scale
                and expert.down_proj.input_scale
            ):
                a1_scale.append(
                    torch.max(
...
class StackExperts(torch.nn.Module):
    def __init__(
        self, hidden_size, intermediate_size, num_expert, dtype, config
    ) -> None:
        super().__init__()
        self.w1 = torch.nn.Parameter(
            torch.empty(num_expert, hidden_size, 2 * intermediate_size, dtype=dtype)
        )
        self.w2 = torch.nn.Parameter(
            torch.empty(num_expert, intermediate_size, hidden_size, dtype=dtype)
        )
