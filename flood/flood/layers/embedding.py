...
class AutoEmbedding:
    ...
    @classmethod
    def from_pretrained(
        cls, config, vocab_size, hidden_size, padding_idx=None, name="embed_tokens"
    ):
        emb_dtype = (
            config.emb_dtype if hasattr(config, "emb_dtype") else None
...
class NativeEmbedding(torch.nn.Module):
    ...
    def retype(self, dtype=torch.float8_e4m3fn):
        if dtype == torch.float8_e4m3fn:
            scale = (
                torch.max(self.weight.data.abs(), dim=1, keepdim=True).values.to(
                    self.model_dtype
                )
                / 448.0
            )
...
class Fp8Embedding(torch.nn.Module):
    def __init__(
        ...
        if weight is None or scale is None:
            weight = torch.empty(vocab_size, hidden_size, dtype=torch.float8_e4m3fn)
    ...
    def forward(self, indices):
        return (
            self.weight.data.view(torch.int8)[indices]
