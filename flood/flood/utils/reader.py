...
class Reader:
    ...
    @staticmethod
    def read_dummy_dataset(
        max_count=1000, input_length=200, output_length=200, flunc=0.0
    ):
        reqs = []
        for i in range(max_count):
            il = input_length + int((2 * random.random() - 1) * flunc * input_length)
            ol = output_length + int((2 * random.random() - 1) * flunc * output_length)
            prompt = "hi" * il
            req = Request(i, input_text=prompt, input_length=il, output_length=ol)
            reqs.append(req)
        Reader.stat(reqs)
        return reqs
