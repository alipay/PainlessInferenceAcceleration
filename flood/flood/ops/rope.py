...
def qk_norm_and_half_rope_forward_kernel(
    ...
    q0 = tl.load(
        q_ptr
        + seq_offsets[:, None, None] * stride
        + DD * head_offs[None, :, None]
        + tl.arange(0, D)[None, None, :],
        mask=mask_seq[:, None, None],
        other=0.0,
    )
    q1 = tl.load(
        q_ptr
    ...
    tl.store(
        qo_ptr
        + seq_offsets[:, None, None] * H * DD
        + D
        + DD * tl.arange(0, H)[None, :, None]
        + tl.arange(0, D)[None, None, :],
        q1,
        mask=mask_seq[:, None, None],
    )
    ...
    qr = tl.reshape(
        tl.permute(
            tl.flip(tl.permute(tl.reshape(q0, (BLOCK, H, 2, d)), (0, 1, 3, 2)), dim=3)
            * signs,
            (0, 1, 3, 2),
        ),
        (BLOCK, H, D),
    ...
    kr = tl.reshape(
        tl.permute(
            tl.flip(tl.permute(tl.reshape(k0, (BLOCK, h, 2, d)), (0, 1, 3, 2)), dim=3)
            * signs,
            (0, 1, 3, 2),
        ),
        (BLOCK, h, D),
...
def qk_norm_and_rope_forward_kernel(
    ...
    q_r = tl.reshape(
        tl.permute(
            tl.flip(tl.permute(tl.reshape(q, (BLOCK, H, 2, d)), (0, 1, 3, 2)), dim=3)
            * signs,
            (0, 1, 3, 2),
        ),
        (BLOCK, H, D),
    ...
    tl.store(
        qo_ptr
        + seq_offsets[:, None, None] * H * D
        + D * tl.arange(0, H)[None, :, None]
        + tl.arange(0, D)[None, None, :],
        q,
        mask=mask_seq[:, None, None],
    )
    ...
    k_r = tl.reshape(
        tl.permute(
            tl.flip(tl.permute(tl.reshape(k, (BLOCK, h, 2, d)), (0, 1, 3, 2)), dim=3)
            * signs,
            (0, 1, 3, 2),
        ),
        (BLOCK, h, D),
...
def triton_qk_norm_and_rope_forward(
    ...
    if rotary_dim == head_dim // 2:
        ...
        raise ValueError(
            f"rotary_dim must be head_dim or head_dim // 2, but got {rotary_dim}"
        )
    return qo, ko, vo
...
def q_k_norm_and_half_rope_forward_kernel(
    ...
    q0 = tl.load(
        q_ptr
        + seq_offsets[:, None, None] * q_stride_n
        + q_stride_h * head_offs[None, :, None]
        + tl.arange(0, D)[None, None, :],
        mask=mask,
        other=0.0,
    )
    ...
    qr = tl.reshape(
        tl.permute(
            tl.flip(
                tl.permute(tl.reshape(q0, (BLOCK, group, 2, d)), (0, 1, 3, 2)), dim=3
            )
            * signs,
            (0, 1, 3, 2),
        ),
    ...
    if head_idx < h:
        ...
        kr = tl.reshape(
            tl.permute(
                tl.flip(tl.permute(tl.reshape(k0, (BLOCK, 2, d)), (0, 2, 1)), dim=2)
                * signs,
                (0, 2, 1),
            ),
            (BLOCK, D),
...
def q_k_norm_and_rope_forward_kernel(
    ...
    q = tl.load(
        q_ptr
        + seq_offsets[:, None, None] * q_stride_n
        + head_offs[None, :, None] * q_stride_h
        + tl.arange(0, D)[None, None, :],
        mask=mask,
        other=0.0,
    )
    ...
    q_r = tl.reshape(
        tl.permute(
            tl.flip(
                tl.permute(tl.reshape(q, (BLOCK, group, 2, d)), (0, 1, 3, 2)), dim=3
            )
            * signs,
            (0, 1, 3, 2),
        ),
    ...
    if head_idx < h:
        ...
        k_r = tl.reshape(
            tl.permute(
                tl.flip(tl.permute(tl.reshape(k, (BLOCK, 2, d)), (0, 2, 1)), dim=2)
                * signs,
                (0, 2, 1),
            ),
            (BLOCK, D),
