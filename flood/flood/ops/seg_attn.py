...
def seg_attn(
    ...
    if MASK_TYPE == 2:
        ...
        mask = tl.load(
            mask_ptrs,
            mask=(offs_n[None, :] >= sub_gap)
            & (offs_n[None, :] < sub_gap + min(MASK_SIZE, (mid + 1) * TOKEN)),
            other=1,
        )
    ...
    if MASK_TYPE != 0:
        if k_acc_length is None:
            qk += tl.where(
                q_pos[:, None] >= (n_offset + offs_n)[None, :], 0, float("-inf")
            )
        else:
            qk += tl.where(
                q_pos >= (k_acc_length + n_offset + offs_n)[None, :], 0.0, float("-inf")
            )
...
def single_seg_attn_kernel(
    ...
    if EVEN_M:
        ...
        q = tl.load(
            q_ptrs, mask=(mid * TOKEN * H + offs_m)[:, None] < max_m_idx, other=0.0
        )
    ...
    if MASK_TYPE != 0:
        ...
        if MASK_TYPE == 2 or MASK_TYPE == 3:
            ...
            if MASK_TYPE == 2:  # mask is broadcasted to qk
                mask_ptrs = (
                    Mask
                    + bid * MASK_SIZE * MASK_SIZE
                    + mid * TOKEN * MASK_SIZE
                    - sub_gap
                    + (tl.arange(0, TOKEN))[:, None] * MASK_SIZE
                    + offs_n[None, :]
                )
                ...
                mask_ptrs = (
                    ...
                    + mid * TOKEN * MASK_SIZE
                    - sub_gap
                    + (tl.arange(0, BLOCK_M) % (GROUP * TOKEN) // GROUP)[:, None]
                    * MASK_SIZE
                    + offs_n[None, :]
                )
    ...
    if EVEN_M:
        ...
        tl.store(out_ptrs, acc_o, mask=(mid * TOKEN * H + offs_m)[:, None] < max_m_idx)
...
def multi_seg_attn_kernel(
    ...
    q = tl.load(q_ptrs, mask=(mid * TOKEN * H + offs_m)[:, None] < max_m_off, other=0.0)
    ...
    if MASK_TYPE == 0:
        ...
        for i_seg in range(n_seg - 1):  # the first n-1 segment
            ...
            k_ptrs = (K + k_offset * stride_k + hid * HEAD_DIM) + (
                offs_n[:, None] * stride_k + offs_d[None, :]
            )
            v_ptrs = (V + k_offset * stride_k + hid * HEAD_DIM) + (
                offs_n[:, None] * stride_k + offs_d[None, :]
            )
        ...
        mask_free_step = (
            min(max(mid * TOKEN + gap - k_acc_length, 0), k_seg_length) // BLOCK_N
        )
    ...
    if EVEN_M:
        ...
        tl.store(out_ptrs, acc_o, mask=(mid * TOKEN * H + offs_m)[:, None] < max_m_off)
...
def seg_attn_fwd(q, k, v, meta, online_scale=True):
    ...
    EVEN_M = all([x % BLOCK_M == 0 and BLOCK_M % GROUP == 0 for x in meta.qls])
    if isinstance(meta.kls[0], (list, tuple)):
        EVEN_N = all([all([y % BLOCK_N == 0 for y in x]) for x in meta.kls])
    else:
